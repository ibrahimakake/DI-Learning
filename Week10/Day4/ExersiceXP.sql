...
select * from film where title = 'Black November'

select  film_id, title, description, length, rental_rate  from film where title ilike 'Bl%'
...
SELECT film.title, inventory.last_update
FROM film
left JOIN inventory 
...
SELECT payment.customer_id, customer.first_name, customer.last_name, payment.amount,payment.payment_date,payment.staff_id
FROM customer
left JOIN payment 
ON customer.customer_id = payment.customer_id
order by staff_id
